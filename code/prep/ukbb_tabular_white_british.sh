...
echo ""
echo $(date) "Join relevant fields into one file..."

# 21000-: Ethnic background
FILE_1=$(find "$DIR_DATA_ACCEL_UKBB_SPLIT_ALL" -type f | grep -e ".*21000-0.0.*" -)
FILE_2=$(find "$DIR_DATA_ACCEL_UKBB_SPLIT_ALL" -type f | grep -e ".*21000-1.0.*" -)
FILE_3=$(find "$DIR_DATA_ACCEL_UKBB_SPLIT_ALL" -type f | grep -e ".*21000-2.0.*" -)

# 22006-0.0: Genetic ethnic grouping
FILE_4=$(find "$DIR_DATA_ACCEL_UKBB_SPLIT_ALL" -type f | grep -e ".*22006-0.0.*" -)

echo $(date) "Join" "$FILE_1", "$FILE_2" "->" "$FILE_JOINED"
join "$FILE_1" "$FILE_2" > "$FILE_TEMP"
mv "$FILE_TEMP" "$FILE_JOINED"

echo $(date) "Join" "$FILE_JOINED", "$FILE_3" "->" "$FILE_JOINED"
join "$FILE_JOINED" "$FILE_3" > "$FILE_TEMP" 
mv "$FILE_TEMP" "$FILE_JOINED"

echo $(date) "Join" "$FILE_JOINED", "$FILE_4" "->" "$FILE_JOINED"
