...
function single (){
    ...
    mkdir -p "$DIR_OUT"
    FILE_LOG="$DIR_OUT""20230529_01_gwas_wrap_""$(date +"%Y%m%d_%H%M%S")"".log"
    bash "$DIR_CODE_ANALYSIS_SPECIFIC""20230529_01_gwas_single.sh" "$FILE_PHENO" "$DIR_OUT" > "$FILE_LOG" 2>&1
}
