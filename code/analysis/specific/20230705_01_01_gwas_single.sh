...
if [ "$MODE" = "large" ]; then
    ...
    # Input: genotype
    PATH_GENO="$DIR_DATA_UKBB_GENOTYPE_WHITE_PAN""merged"
    # Input: covariates
    FILE_COVAR="$DIR_DATA_ACCEL_UKBB_COVAR_WHITE_PAN""covar_age_sex_combination_pc10.txt"
...
if [ "$MODE" = "small" ]; then
    # Input: PLINK2-related
    # Input: genotype
    PATH_GENO="$DIR_DATA_UKBB_GENOTYPE_WHITE_QC""merged"
    # Input: covariates
    FILE_COVAR="$DIR_DATA_ACCEL_UKBB_COVAR_WHITE_QC""covar_age_sex_combination_pc10.txt"
    # MAGMA batch job number
    # Theoretical limit is 29 or 30
    # Practical limit is much lower due to machine memory size
    N_BATCH_MAGMA=10
