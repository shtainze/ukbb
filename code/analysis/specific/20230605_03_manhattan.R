...
manhattan_part <- function(df, mode, start, end, thin_interval, filename) {
	...
	if (mode=="part"){
		...
		manhattan(df, xlim=c(start, end), ylim=c(0, 20), logp=TRUE, genomewideline = -log10(5e-08),
...
dir_root <- paste("..", "..",	"..", 
	"analysis", "specific", "analysis_20230605_01_gwas", "1180-0.0_Chronotype_fourway_small",
	sep="\\")

file_source = paste(dir_root, "for_manhattan_chr1.txt", sep="\\")
df_chr1 <- read.table(file_source, stringsAsFactors=F, sep = " ", header = TRUE, na.strings = "")

file_source = paste(dir_root, "for_manhattan_chr11.txt", sep="\\")
df_chr11 <- read.table(file_source, stringsAsFactors=F, sep = " ", header = TRUE, na.strings = "")

file_source = paste(dir_root, "for_manhattan.txt", sep="\\")
